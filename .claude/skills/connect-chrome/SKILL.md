...
description: |
  ...
  Supports multiple simultaneous Chromium instances on different ports (34567+).
  Use when asked to "connect chrome", "open chrome", "launch browser",
  "open gstack browser", "real browser", "launch chrome", or "control my browser".
  Voice triggers (speech-to-text aliases): "connect chrome", "show me the browser".
triggers:
  - connect chrome
  - open chrome
  - launch browser
...
---
<!-- Forked from gstack open-gstack-browser — safe to edit directly -->
...
## Step 0: Pre-flight cleanup (parallel-agent safe)

Each /connect-chrome invocation runs in its own port AND its own Chromium
profile dir (`~/.gstack/chromium-profile-<port>`). Just sweep abandoned lock
files for profile dirs whose owning Chromium is gone — never touch profiles
owned by another live agent.

```bash
for _DIR in "$HOME/.gstack"/chromium-profile-*; do
  [ -d "$_DIR" ] || continue
  _LIVE=0
  if command -v pgrep >/dev/null 2>&1; then
    pgrep -f "user-data-dir=$_DIR" >/dev/null 2>&1 && _LIVE=1
  fi
  if [ "$_LIVE" = "0" ]; then
    for _LF in SingletonLock SingletonSocket SingletonCookie; do
      rm -f "$_DIR/$_LF" 2>/dev/null || true
    done
  fi
done
echo "Pre-flight cleanup done (per-port profiles; live ones left alone)"
```

## Step 1: Connect (auto-port)

```bash
# Auto-find a free port starting at 34567 (parallel-agent safe)
_PORT=34567
while (netstat -ano -p TCP 2>/dev/null || netstat -an 2>/dev/null) | awk '/LISTEN/ {print $4}' | grep -q "[:.]$_PORT$"; do
  _PORT=$((_PORT + 1))
  [ "$_PORT" -gt 34600 ] && { echo "ERROR: no free port in 34567-34600"; exit 1; }
done
echo "BROWSE_PORT=$_PORT (auto-selected)"
# Per-agent isolation: profile dir + state file keyed off the port.
export BROWSE_STATE_FILE="$HOME/.gstack/browse-$_PORT.json"
BROWSE_PORT=$_PORT $B connect
```
...
**Each agent runs in full isolation:** port 34567+, profile dir
`~/.gstack/chromium-profile-<port>/`, state file `~/.gstack/browse-<port>.json`.
The sidebar extension hardcodes auto-connect on port **34567** — on any other
port the badge stays gray until the user types the port in the gear icon.
...
```bash
cat "$BROWSE_STATE_FILE" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```

The port should match `$_PORT` from Step 1 — usually 34567, possibly 34568+ if
another agent already held 34567. Tell the user the port so they can enter it
manually in the sidebar extension's gear icon if auto-connect fails.
